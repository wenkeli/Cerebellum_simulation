...
qmake -project -t lib INCLUDEPATH+="$INC_PATH" LIBS+="$LIB_PATH $LIBS" CONFIG="release" DESTDIR="../libs" OBJECTS_DIR="intout" CUDA_SOURCES=$CUDA_SOURCES -o cbm_core.pro

# Add this text to the bottom of the project file to tell qmake how to compile cuda files
echo NVCCFLAGS = -O3 -arch=compute_20 -Xcompiler -fPIC >> cbm_core.pro
