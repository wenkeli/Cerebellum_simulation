...
//		cout<<threshIO[i]<<" ";
		...
		as->vIO[i]=as->vIO[i]+
				...
				gNCSum*1.5*(ap->eNCtoIO-as->vIO[i])+
				as->vCoupleIO[i]+
				as->errDrive;
...
#pragma omp for
		for(int i=0; i<cp->numNC; i++)
		{
			...
			for(int j=0; j<cp->numpNCfromPCtoNC; j++)
			{
				...
				as->gPCNC[i][j]=as->gPCNC[i][j]*ap->gDecPCtoNC+as->inputPCNC[i][j]*as->gPCScaleNC[i][j]*(1-as->gPCNC[i][j]);
				gPCNCSum=gPCNCSum+as->gPCNC[i][j];
			}
			gPCNCSum=gPCNCSum*ap->msPerTimeStep/((float)cp->numpNCfromPCtoNC);
	//		cout<<gMFNMDASum<<" "<<gMFAMPASum<<" "<<gPCNCSum<<" "<<inputMFNCSum<<" "<<inputPCNCSum<<endl;

	//		cout<<(int)as->inputMFNC[i][0]<<" "<<as->mfSynWeightNC[i][0]<<" "<<as->mfNMDANC[i][0]<<" "<<as->inputMFNC[i][0]*as->mfSynWeightNC[i][0]*(1-as->mfNMDANC[i][0])<<" "<<as->gMFNMDANC[i][0]<<" "<<as->mfAMPANC[i][0]<<" "<<as->gMFAMPANC[i][0]<<endl;
...
//		as->pfPCPlastTimerIO[i]=(!as->apIO[i])*(as->pfPCPlastTimerIO[i]+1)+as->apIO[i]*(-100);
		as->pfPCPlastTimerIO[i]=(!as->apIO[i])*(as->pfPCPlastTimerIO[i]+1)+as->apIO[i]*ap->tsLTDStartAPIO;
		as->vCoupleIO[i]=0;
