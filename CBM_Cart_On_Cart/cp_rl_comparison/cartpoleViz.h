#ifndef CARTPOLE_VIZ_H_
#define CARTPOLE_VIZ_H_
...
{
 public:
    CartPoleViz(float _tracklen, float _polelen, float _leftAngleBound, float _rightAngleBound,
    ...
    void drawCartpole(float cartpos, float cartvel, float poleang, float polevel,
                      ...
                      int timeAloft, int trialNum, int cycle, int playspeed=1);
 ...
 protected:
    ...
    static void *thread_helper(void *context) {
      ((CartPoleViz *)context)->SDL_Mainloop();
    }
    void renderScreen();
 ...
 protected:
    ...
    int timeAloft, trialNum, cycle, playspeed;
    ...
    static const SDL_Rect purple_reverse, purple_back, purple_ff, purple_next, purple_play, purple_pause;
    static const SDL_Rect speedup, slowdown;
};
