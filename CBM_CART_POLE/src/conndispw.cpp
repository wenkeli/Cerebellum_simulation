...
void ConnDispW::paintBuf()
{
	...
	if(dispT==MFGR)
	{
		...
		{
			for(int j=0; j<numMFtoGRN[i]; j++)//for(int j=0; j<numSynMFtoGR; j++)
			{
				int grNum=conMFtoGRN[i][j]/4;
	...
	if(dispT==MFGO)
	{
		...
		for(int i=start; i<=end; i++)
		{
			for(int j=0; j<numMFtoGON[i]; j++)//for(int j=0; j<numSynMFtoGO[i]; j++)
			{
//				int goNum=conMFtoGO[i][j];
				int goNum=conMFtoGON[i][j];
//				if(goNum>=NUMGO)
//					cout<<goNum<<endl;
	...
	if(dispT==GRGO)
	{
		...
		for(int i=start; i<=end; i++)
		{
			...
			{
				for(int k=0; k<numSynGRtoGO[j]; k++)
				{
	...
	if(dispT==GOGR)
	{
		...
		{
			painter.fillRect((int)(((i%GOX)+0.5)/scaleX-1), (int)((((int)i/GOX)+0.5)/scaleY-1), 3, 3, Qt::red);
			for(int j=0; j<numGOtoGRN[i]; j++) //for(int j=0; j<numSynGOtoGR[i]; j++)
			{
//				int grNum=conGOtoGR[i][j]/4;
				int grNum=conGOtoGRN[i][j]/4;
